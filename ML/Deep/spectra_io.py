...
def segment_normalize(
    ...
    """Continuum-normalise by fitting polynomials in ~seg_width Angstrom segments.

    Less aggressive than Gaussian-kernel convolution — narrow absorption
    features (including CN bands) are better preserved.
    """
    n = len(flux)
    ...
    norm = np.empty_like(flux, dtype=np.float32)
    ...
    return np.clip(norm, 0.0, 3.0).astype(np.float32)
...
def normalize_datacube_segment(
    ...
    for i in iterator:
        out[i] = segment_normalize(datacube[i], wave)
    return out
...
def normalize_datacube(
    ...
    ----------
    norm_mode : str
        "segment" — polynomial fit in ~200A windows (default, preserves narrow features).
        "gauss"   — Gaussian kernel convolution (legacy, can smooth CN bands).
    common_wave : np.ndarray or None
        Required when norm_mode="segment".
    """
    if datacube.ndim != 2:
        raise ValueError("datacube must be 2D: (n_spectra, n_wave)")

    if norm_mode == "segment":
        if common_wave is None:
            raise ValueError("common_wave is required for norm_mode='segment'")
        out = normalize_datacube_segment(datacube, common_wave)
    else:
        out = np.asarray(
            [
...
def run_load_normalize_pipeline(
    ...
    datacube_norm, good_mask = normalize_datacube(
        datacube_raw, norm_mode=norm_mode, common_wave=common_wave)
...
def run_screening_preprocess_pipeline(
    ...
    ----------
    norm_mode : str
        "segment" (default) — polynomial segment fitting, preserves narrow features.
        "gauss" — Gaussian kernel convolution (legacy).
    """
